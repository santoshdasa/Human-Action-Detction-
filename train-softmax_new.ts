...
const upperhit='upperhit-aug';
const jump='jump-aug';
const squat='squat-aug';
...
const train = async () => {
  ...
  const kicks = require('fs')
    ...
    const squat = require('fs')
    .readdirSync(squat)
    .filter(f => f.endsWith('.jpg'))
    .map(f => `${squat}/${f}`);

    const jump = require('fs')
    .readdirSync(jump)
    .filter(f => f.endsWith('.jpg'))
    .map(f => `${jump}/${f}`);

    const upperhit = require('fs')
    .readdirSync(upperhit)
    .filter(f => f.endsWith('.jpg'))
    .map(f => `${upperhit}/${f}`);
  ...
  const ys = tf.tensor2d(
    new Array(hits.length)
      .fill([1, 0, 0, 0, 0, 0])
      .concat(new Array(kicks.length).fill([0, 1, 0,0,0,0]))
      .concat(new Array(squat.length).fill([0, 0, 1,0,0,0]))
      .concat(new Array(jump.length).fill([0, 0, 0,1,0,0]))
      .concat(new Array(uppperhit.length).fill([0, 0, 0,0,1,0]))
      .concat(new Array(negatives.length).fill([0, 0, 0,0 , 0 ,1])),
    [hits.length + kicks.length + squat.length + jump.length + upperhit.length + negatives.length, 6]
  );
  ...
  const xs: tf.Tensor2D = tf.stack(
    hits
      ...
      .concat(squat.map((path: string) => mobileNet(readInput(path))))
      .concat(jump.map((path: string) => mobileNet(readInput(path))))
      .concat(upperhit.map((path: string) => mobileNet(readInput(path))))
  ...
  await model.fit(xs, ys, {
    epochs: Epochs,
    batchSize: parseInt(((hits.length + kicks.length + squat.length + jump.length + upperhit.length + negatives.length) * BatchSize).toFixed(0)),
    callbacks: {
